...
def get_notes(measure, staff_n):
    """Return all list of all notes and rests in a given measure and staff."""
...
def add_wrapper_to_staff(staff, skip, duration, wrapperlist, ALT_TYPE):
    logging.debug('add_wrapper_to_staff(): ' +
                  staff.getAttribute('n').value + ', s' +
                  str(skip) + 'd' + str(duration))
    ...
    construct. The location of the <app> element is specified by the skip
    paramter, and the length of the <app> element is defined by the dur
    parameter. The parameters skip and duration refer to the length in
    semibreves of the notes occurring before the <app> element is
    to begin, and the length in semibreves of the <app> element. We assume
    that there is only one layer in the staff given.

    The wrapperlist paramter is an in/out parameter. The created <app> object
    is stored in wrapperlist. The wrapperlist paramter is a dictionary, and
    it is indexed by the location of the app elements within the measure.
    There can only be one <app> element at a given location. If there's
    already an <app> element defined at the given location, the method
    returns without making any modification to the staff.
    ...
    if skip not in wrapperlist:
        logging.debug("There isn't any wrapper at position " +
                      str(skip) + " yet. Creating wrapper.")
        ...
        skip__ = skip
        logging.debug("Skipping notes in lemma that are before position " +
                      str(skip) + ":")
        for note in layer_notes:
            ...
            # If the skip has been exhausted, begin the duration:
                ...
                if skip not in wrapperlist:
                    logging.debug('adding new rich_wrapper at SKIP' +
                                  str(skip))
        ...
        if skip not in wrapperlist:
            raise RichWrapperInsertError(staff, skip, duration,
                                         ALT_TYPE, rich_wrapper)
...
def legal_overlapping(staff, skipdurs):
    ...
    def legal_with_lemma(staff, skip, dur):
        ...
        """
        logging.debug("legal_with_lemma(" + str(staff) + ", " +
                      str(skip) + ", " + str(dur) + ")")
        logging.debug('legal_with_lemma(): ' + staff.getAttribute('n').value +
                      ', s' + str(skip) + 'd' + str(dur))
        old_layers = staff.getChildrenByName('layer')
        if len(old_layers) == 0:
            return False
    ...
    def legal_with_each_other(skipA, durA, skipB, durB):
        ...
        # Case when one really has no dur (shouldn't happen?)
            ...
            logging.debug('Not legal with each-other: s' + str(skipA) +
                          'd' + str(durA) + ', and ' + 's' + str(skipB) +
                          'd' + str(durB))
            return False
    ...
    if vl == []:
        ...
        if vl[0][2] == lemma_n:
            ...
            if len(layers) > 0:
                ...
                notelist = []
            answer = [(staff,
                      get_colored_blocks_from_notes(notelist, color_we_want))]
        else:
            answer = []
        return answer + get_colored_blocks(measure, lemma_n,
                                           vl[1:], color_we_want)
...
def add_rich_elems(measure, alternates_list, color_we_want, ALT_TYPE):
    """Same as add_all_apps_in_measure, but using
    colored_blocks instead of (skip, dur) tuples
    """
    ...
    for L in lemmas:
        ...
        colored_blocks = get_colored_blocks(measure, L,
                                            alternates_list, color_we_want)
        logging.debug('All colored blocks for Lemma ' + str(L) +
                      ': ' + str(colored_blocks))
        staff = get_staff(measure, L)
        # TODO: merge sources where they coincide! --
        # HERE? or after having looked up source IDs?
        ...
        if legal_overlapping(staff, flat_list_of_colored_blocks):
            ...
            for cbs in colored_blocks:
                ...
                for cb in cbs[1]:
                    ...
                    notelist = cb[3]
                    try:
                        rich_wrapper = add_wrapper_to_staff(staff, skip, dur,
                                                        wrapperlist, ALT_TYPE)
                        # add rdg elements with reference to notelist,
                        # but do not insert notelist yet.
...
def merge_identical_colored_blocks(all_colored_blocks):
    """Reduce the number of distinct colored blocks by merging
    together identical blocks. As a result, a new (list of
    (varstaff, (list of (color, skip, dur, notelist))) is
    created. In the new set of colored blocks the following
    is true:
      - if A and B two colored blocks
           AND A.skip == B.skip
           AND A.color == B.color
        THEN A.notelist != B.notelist
    """
    # list of (varstaff, colored_blocks_by_skip)
    # where colored_blocks_by_skip[skip][color]
...
def remove_measure_staves(measure, alternate_list):
    ...
    for list_item in alternate_list:
        for staff in staff_list:
            if (staff.hasAttribute('n') and
                    staff.getAttribute('n').value == list_item[0]):
...
def delete_staff_def(MEI_tree, variants_list):
    ...
    for variants_list_item in variants_list:
        for staffDef in staffDefs:
            if (staffDef.hasAttribute('n') and
                    staffDef.getAttribute('n').value == variants_list_item[0]):
...
def local_alternatives(MEI_tree, alternates_list, color_we_want, ALT_TYPE):
    ...
    for measure in MEI_tree.getDescendantsByName('measure'):
        ...
        add_rich_elems(measure, filtered_alternates_list,
...
def corresponding_wrappers(rich_wrapper1, rich_wrapper2):
    ...
    for rich_item in rdg1s:
        ...
        result = (result and
                  rich_attr_val != '' and
                  len(get_descendants(rich_wrapper2, rich_item_name + '[' + rich_attr_name + '=' + rich_attr_val + ']')
                      ) == 1
                  )
    return result
...
def find_connecting_wrapper(rich_wrapper):
    ...
    A connecting app in the next measure has the following properties:
      * it is on the same staff
      * it is at the beginning of the measure (no notes before)
      * it has the same set of sources than 'app'
    ...
    for elem in layer_descendants:
        if not found and elem == rich_wrapper:
            ...
            if (not elem in rich_wrapper.getDescendants() and
                    dur_in_semibreves(elem) > 0):
    ...
    for elem in layer_children:
        ...
        if elem.getName() != rich_wrapper_name and dur_in_semibreves(elem) > 0:
            logging.debug('elem.getName():' + elem.getName() +
                          ' and dur_in_semibreves(elem) > 0: ' +
                          str(dur_in_semibreves(elem))
                          )
            return None
        elif (elem.getName() == rich_wrapper_name and
                corresponding_wrappers(rich_wrapper, elem)):
            return elem
...
def link_alternatives(MEI_tree, ALT_TYPE):
    """Link together variants or emendations that span
    across measures using the <annot> elements
    """
    ...
    for rich_wrapper in get_descendants(MEI_tree, rich_wrapper_name):
        logging.debug("searching for connecting apps of app: " +
                      str(rich_wrapper))
        ...
        if connecting_wrapper:
            ...
            group_ID_with_refID(annot_groups, rich_wrapper.getId(),
                                connecting_wrapper.getId())
